...
exports.getUserInfo = (req, res) => {
    ...
    const sql = `select id, username, nickname, email, user_pic from am_users where id=?`
...
exports.updateUserInfo = (req, res) => {
    ...
    const sql = `update am_users set ? where id=?`
...
exports.updatePassword = (req, res) => {
    // 根据 id 查询用户的信息
    const sql = `select * from am_users where id=?`
    ...
    db.query(sql, req.user.id, (err, results) => {
        ...
        const sql = `update am_users set password=? where id=?`
            // 对新密码进行加密处理
...
exports.updateAvatar = (req, res) => {
    // 1. 定义更新头像的 SQL 语句
    const sql = `update am_users set user_pic=? where id=?`
